...
const toggleSubscription = asyncHandler(async (req, res) => {
  ...
  if (existingSubscription) {
    ...
    if (!deletedSubscription) {
      throw new apiError(
        500,
        "There was a problem while toggling subscription"
      );
    }
    return res
      .status(200)
      .json(new apiResponse(200, null, "Successfully unsubscribed"));
  }
...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new apiResponse(200, { subscribers }, "Successfully sent subscribers")
    );
});
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
  ...
  const subscribedChannels = await Subscription.aggregate([
    {
      $match: {
        $expr: {
          $eq: ["$subscriber", { $toObjectId: subscriberId }]
        }
      }
    },
    {
  ...
  if (subscribedChannels === 0) {
    return res
      .status(200)
      .json(
        new apiResponse(200, subscribedChannels, "No channels subscribed yet!")
      );
  }
